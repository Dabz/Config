...
function! s:searchForTemplate(class, companion)
  let companion = FSReturnReadableCompanionFilename('%')
  let prev_line = "FIRST LINE"
  for line in readfile(companion)
    if (match(line, "class") != -1 && match(line, a:class) != -1)
      if (match(prev_line, "template.*<.*>") != -1)
	return matchstr(prev_line, "<.*>")
      endif
    endif
...
function! s:GetFunctionPrototypesForCurrentBuffer(opts)
  ...
  if companion != ''
	...
	if part4 == 'class'
	  ...
	  let template = s:searchForTemplate(class, companion)
	  if includeNS == 0
	...
	call add(commands, linenum . '|' . fname . '|' . class . '|' . template)
      endif
    endfor
...
augroup protodef_cpp_mapping
  au! BufEnter *.cpp,*.C,*.cxx,*.cc,*.CC,*.hxx call protodef#MakeMapping()
augroup END
